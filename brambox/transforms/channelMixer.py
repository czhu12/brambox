...
class ChannelMixer:
    ...
    def get_output_image(self):
        ...
        if m >= len(self.in_img):
            raise ValueError("One of your channels references an image that is not available (%d)" % m)
        ...
        for c in self.channels:
            ...
            if len(shape) < 3:
                ...
                c_count = shape[2]
            if c[1] >= c_count:
                raise ValueError("One of your channels references a channel that does not exist in the input image (%d,%d)" % (c[0], c[1]))
